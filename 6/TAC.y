...
line: /* empty */
    ...
    | WHILE TERM DO '{' { printf("LABEL%d: if not %s then FALSE%d\nTRUE%d: ", cc, $2, cc, cc); } line '}' { printf("FALSE%d: ", cc); cc++; } line
...
cases: /* empty */
     | CASE TERM ':' { printf("CASE%d: if t%d != %s goto CASE%d\n", case_count, sc, $2, case_count + 1); case_count++;} line cases
     | DEFAULT{printf("CASE%d: ", case_count);} ':' line { printf("goto NEXT%d\n", cc); } cases
%%
int yyerror(char* s)
{
  fprintf(stderr, "%s\n", s);
